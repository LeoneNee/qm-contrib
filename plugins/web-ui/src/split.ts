...
function splitPane(paneId: string, edge: SplitEdge, params: PaneParams): void {
  ...
  if (dockApi.groups.length >= MAX_TILES) {
    if (tabIntoPane(paneId, params)) canvasToast(t("{count} tiles is the limit — opened as a tab", { count: MAX_TILES }));
    return;
  }
...
export function reparentCanvasHost(target: HTMLElement): void {
  if (!canvasHost) return;
  if (canvasHost.parentElement !== target) {
    target.replaceChildren(canvasHost);
    dockApi?.layout(window.innerWidth, window.innerHeight, true);
  }
  for (const c of paneContents.values()) c.syncZones();
  for (const c of paneContents.values()) c.conversation.reattachHost();
  for (const c of paneContents.values()) c.syncTitle();
}
