...
import { currentLocale, onLocaleChange, setLocale, t } from "./i18n.ts";
...
onLocaleChange(() => {
  ...
  const viewingChat = mainConversation().state.host?.isConnected === true;
  mountShell();
  restoreLocaleRerender(viewingChat);
});

function restoreLocaleRerender(viewingChat: boolean): void {
  if (appState.currentView !== "chats") {
    refreshActiveView(appState.currentView);
    return;
  }
  if (splitState.active) {
    if (appState.mainEl) reparentCanvasHost(appState.mainEl);
    drawCanvas();
    return;
  }
  if (viewingChat) {
    mainConversation().reattachHost();
    renderList();
    return;
  }
  void renderChatsPage();
  renderList();
}
...
function devGate(gate: { value?: string; error?: string; pending?: boolean }) {
  return gateShell(html`
    ...
    >
      <h1>${t("Dev sign-in")}</h1>
      <p class="signin-body">
        ${t("No identity provider is configured, so this instance trusts a local cookie. Set")}
        <b>CORE_SIGNING_SECRET</b> ${t("and run the portal to use real sign-in.")}
      </p>
      ...
      <div class="layout ${sidebarOpen ? "" : "sidebar-closed"} ${banner ? "bannered" : ""}">
        <aside class="sidebar" aria-label="Navigation" @keydown=${onSidebarKeydown}>
          ...
          <div class="sidebar-footer">
            ...
            >
            <button
              class="icon-btn subtle lang-toggle"
              type="button"
              aria-pressed=${currentLocale() === "zh" ? "true" : "false"}
              title=${t("Language: 中文 / English") + " — " + (currentLocale() === "zh" ? t("Switch to English") : t("Switch to 中文"))}
              aria-label=${t("Language: 中文 / English") + " — " + (currentLocale() === "zh" ? t("Switch to English") : t("Switch to 中文"))}
