        ...
        <div class="kc-resource-copy">
          ...
          <div class="kc-credential-facts">
            <div class="kc-audit-line">
              ${icon(Activity, 14)}${
                lastUse
                  ? html`${t("Last used {date} in {scope} · {status}", {
                      date: fmtDate(lastUse.ts),
                      scope: scopeName(lastUse.scopeLabel),
                      status: lastUse.status,
                    })}`
                  : t("No audited use yet")
              }
            </div>
                ...
                (ask) =>
                  html`<div class="kc-access-row">
                    <div>
                      ...
                      ${t("requested {mode} access", {
                        mode: ask.requestedMode === "standing" ? t("standing") : t("one-time"),
                      })}
                ...
                (grant) =>
                  html` <div class="kc-access-row">
                    <div>
                      ...
                      ${grant.mode === "standing" ? t("standing") : t("once")}
...
async function deleteCredential(credential: KeychainCredential): Promise<void> {
  ...
  const impact = active.length
    ? ` ${t(
        active.length === 1
          ? "It will immediately revoke {count} active grant: {list}."
          : "It will immediately revoke {count} active grants: {list}.",
        { count: active.length, list: active.map((grant) => scopeName(grant.audienceScopeId)).join(", ") },
      )}`
    : "";
...
async function revokeGrant(grant: KeychainGrant): Promise<void> {
  ...
  confirmation = {
    ...
    body: t("This {mode} access ends immediately. Automations using it may stop working.", {
      mode: grant.mode === "standing" ? t("standing") : t("one-time"),
    }),
