    ...
    for (const entry of await readAllEntries(dir)) {
      ...
      if (out.length >= caps.maxFiles) {
        throw new FolderDropError(
          t('"{name}" has too many files (over {max}) — zip it yourself or drop a subfolder.', {
            name: root.name,
            max: caps.maxFiles.toLocaleString(),
          }),
        );
      }
      ...
      if (bytes > caps.maxBytes) {
        throw new FolderDropError(
          t('"{name}" is too big (over {mb} MB) — zip it yourself or drop a subfolder.', {
            name: root.name,
            mb: Math.round(caps.maxBytes / (1024 * 1024)),
          }),
        );
      }
