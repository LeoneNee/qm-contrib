    ...
    <div class="skill-variant ${archived ? "archived" : ""}">
      ...
      <div class="skill-variant-copy">
        ...
        <div class="skill-variant-meta">
          ${skillMeta(s)}${s.assetCount ? ` · ${s.assetCount === 1 ? t("1 asset") : t("{count} assets", { count: s.assetCount })}` : ""}
        </div>
        ...
        reviewed
          ? html`<div class="skill-impact" role="alert">
              ...
              <div class="card-meta">
                ...
                ${t("instructions {status}", { status: e.body === e.originalBody ? t("unchanged") : t("changed") })}.
              </div>
            </div>`
          : nothing
      }
    ...
    html`${listPageTpl({
      ...
      filters: html`<div class="skill-registry-controls">
        ...
        <div class="skill-result-count" aria-live="polite">
          ${loading ? t("Loading…") : t("{skills} in {groups}", {
            skills: filtered.length === 1 ? t("1 skill") : t("{count} skills", { count: filtered.length }),
            groups: groups.length === 1 ? t("1 group") : t("{count} groups", { count: groups.length }),
          })}
        </div>
  ...
  >
    ...
    >
      ...
      <p id="skill-archive-impact">
        ${t("This version will stop being available to {audience}.", { audience })}
        ${t("If it overrides a broader /{name}, that version becomes effective.", { name: skill.name })}
        ${t("Its history and assets are kept, and you can restore it later.")}
      </p>
