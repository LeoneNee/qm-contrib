...
export function backgroundLabel(
  ...
  const parts: string[] = [];
  if (jobs > 0)
    parts.push(jobs === 1 ? t("1 background job running") : t("{count} background jobs running", { count: jobs }));
  if (watches > 0) parts.push(watches === 1 ? t("1 watch armed") : t("{count} watches armed", { count: watches }));
