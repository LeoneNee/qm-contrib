...
function listWhen(ms: number): string {
  ...
  return new Date(ms).toLocaleDateString(intlLocale(), { month: "short", day: "numeric" });
}
  ...
  return html`${ind.working ? html`<span class="working-dot" ${ref(syncWorkingPulse)} title=${t("Agent is working")} aria-label=${t("Agent is working")}></span>` : nothing}${
    ...
    ind.background
      ? html`<span
          ...
          @mouseenter=${(e: Event) =>
            showTooltip(e.currentTarget as Element, t("{label} — click to inspect", { label: ind.background!.label }))}
          @mouseleave=${(e: Event) => hideTooltip(e.currentTarget as Element)}
          @focus=${(e: Event) => showTooltip(e.currentTarget as Element, t("{label} — click to inspect", { label: ind.background!.label }))}
