...
const ref = sep === -1 ? "" : c.ownerScopeId.slice(sep + 1);
  if (kind === "channel") return c.scopeName ? `#${c.scopeName}` : t("a Slack channel", ref);
    ...
    html`
      <div class="resource-detail">
        ...
        <div class="field">
          ...
          <div class="value">${next != null ? new Date(next).toLocaleString(intlLocale()) : "—"}</div>
        </div>
        <div class="field">
          <label>${t("Last fired")}</label>
          <div class="value">${c.lastFiredAt ? new Date(c.lastFiredAt).toLocaleString(intlLocale()) : t("Never")}</div>
        </div>
    ...
    <div class="cron-run-list">
      ${[...runs].reverse().map((run) => {
        ...
        return html` <div class="cron-run-row">
          ...
          <span class="cron-run-time">${new Date(run.firedAt).toLocaleString(intlLocale())}</span>
...
function cronDialogTpl(dialog: { kind: "rename" | "delete"; cron: CronView }): TemplateResult {
  ...
  >
    ...
    >
      ...
      <p class="hint">
        ${t("To change {what}, use the agent so it can validate the resulting behavior and permissions.", {
          what:
            c.message === undefined
              ? t("the schedule, timezone, destination, or run mode")
              : t("the message, schedule, timezone, destination, or run mode"),
        })}
      </p>
  ...
  return html`
    <form class="resource-form cron-form" @submit=${onCreateCron}>
      ...
      <p class="hint">
        ...
        <code>Gmail unread digest</code> ${t("or")} <code>GitLab CI watch</code>${t(".")}
      </p>
      <label>
        <textarea
          name="text"
