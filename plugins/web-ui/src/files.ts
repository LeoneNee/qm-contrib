...
async function uploadFiles(files: globalThis.File[]): Promise<void> {
  ...
  filesUploading = true;
  filesNotice =
    picked.length === 1 ? t("Uploading {count} file…", { count: 1 }) : t("Uploading {count} files…", { count: picked.length });
  drawFiles();
  let uploaded = 0;
  try {
    ...
    filesNotice =
      picked.length === 1 ? t("Uploaded {count} file.", { count: 1 }) : t("Uploaded {count} files.", { count: picked.length });
    await loadFiles(appState.viewRenderSeq);
  } catch (e) {
    filesNotice = `${uploaded ? `${t("Uploaded {done} of {total}.", { done: uploaded, total: picked.length })} ` : ""}${errMessage(
      e,
      t("Upload failed."),
    )}`;
